# ============================================================
# PCA Analysis of Metabolomic Data Across LC-MS Panels
#
...
#    for differences in overall signal intensity.
...
#
# This analysis is used to evaluate how pool size influences 
# metabolomic profile structure across datasets.
# ============================================================

# set working directory
# This script expects to be run with the working directory set to
# Experiment 2/Normalization/ within the repository. It reads
# Data_raw.csv and Meta.csv from this folder, and writes normalized
# per-sample output and the multi-panel PCA figure to
# Mean_centered_by_Sample/ within this folder.
...
df_Hpos_norm[, 9:ncol(df_Hpos_norm)] <- t(scale(
  t(df_Hpos_norm[, 9:ncol(df_Hpos_norm)]),
  center = TRUE,
  scale = FALSE
))
...
df_Hneg_norm[, 9:ncol(df_Hneg_norm)] <- t(scale(
  t(df_Hneg_norm[, 9:ncol(df_Hneg_norm)]),
  center = TRUE,
  scale = FALSE
))
...
df_C18neg_norm[, 9:ncol(df_C18neg_norm)] <- t(scale(
  t(df_C18neg_norm[, 9:ncol(df_C18neg_norm)]),
  center = TRUE,
  scale = FALSE
))
...
df_C18pos_norm[, 9:ncol(df_C18pos_norm)] <- t(scale(
  t(df_C18pos_norm[, 9:ncol(df_C18pos_norm)]),
  center = TRUE,
  scale = FALSE
))
