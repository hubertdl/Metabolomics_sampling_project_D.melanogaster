...
# ============================================================
# Downsampling Figures and Signal-Detection Power Model
#
# Reads the per-iteration linear model results produced by
# DownSamplingByPoolSizeLM_capped_resampling.R (one row per
# metabolite per downsampling iteration) and uses them to:
#
#   1. Model how the total number of diet-significant metabolites
#      detected per iteration depends on replicate number and pool
#      size, using a zero-inflated Poisson GLMM (model_power_zip).
#   2. Generate figures summarizing true/false positive recovery
#      across replicate and pool-size downsampling, effect-size
#      concordance between pool sizes at full replication, and
#      signal retention stratified by effect-size bin.
#
# Figures and summary tables are written to DownSamplingXPool_Figures/.
# ============================================================
...
# on your local machine). It reads its input from the
# DownSamplingXPool_Results_CappedResampling/ subfolder produced by
# DownSamplingByPoolSizeLM_capped_resampling.R, and writes figures
# and output tables to DownSamplingXPool_Figures/, both relative to
# this location.
...
# =========================
files <- list.files(
  "DownSamplingXPool_Results_CappedResampling//",
  pattern = "\\.csv$",
  full.names = TRUE
)
...
#
# Diet_sig_n is a bounded count (number of significant
# metabolites out of the total in a panel), not a continuous,
# unbounded quantity, so this is fit as a count GLMM rather than a
# Gaussian LMM. Model-building sequence and why:
#
#   1. model_power_pois (Poisson): baseline count model.
#   2. model_power_nb (negative binomial): tested whether Poisson's
#      constant-dispersion assumption was violated. LRT against
#      Poisson was non-significant (p = 0.80) and the NB
#      dispersion test itself came back near 1 (0.95), so the
#      extra dispersion parameter isn't needed once the Dataset
#      random intercept is in the model.
#   3. model_power_zip (zero-inflated Poisson): DHARMa's zero-
#      inflation test on model_power_pois showed the data have
#      ~13.5x more zero-detection iterations than the Poisson
#      model expects (p < 2.2e-16), concentrated at pool size 5.
#      Adding ziformula = ~ PoolSize resolved this (ratioObsSim
#      dropped to 1.17) and improved AIC by 720 points
#      (LRT chi2 = 725, df = 3, p < 2.2e-16). This is the final
#      model.
# =========================
model_power_pois <- glmmTMB(
  Diet_sig_n ~ N_remaining_num * PoolSize + (1 | Dataset),
  data = summary_results_df,
  family = poisson(link = "log")
)

model_power_nb <- glmmTMB(
  Diet_sig_n ~ N_remaining_num * PoolSize + (1 | Dataset),
  data = summary_results_df,
  family = nbinom2(link = "log")
)

# NB dispersion parameter not justified over Poisson (see header note)
anova(model_power_pois, model_power_nb)

model_power_zip <- glmmTMB(
  Diet_sig_n ~ N_remaining_num * PoolSize + (1 | Dataset),
  ziformula = ~ PoolSize,
  data = summary_results_df,
  family = poisson(link = "log")
)
# zero-inflation term strongly justified over plain Poisson
anova(model_power_pois, model_power_zip)

# interaction term (replicate number x pool size) also strongly
# justified: chi2 = 415.7, df = 2, p < 2.2e-16
model_power_zip_additive <- glmmTMB(
  Diet_sig_n ~ N_remaining_num + PoolSize + (1 | Dataset),
  ziformula = ~ PoolSize,
  data = summary_results_df,
  family = poisson(link = "log")
)
anova(model_power_zip_additive, model_power_zip)

summary(model_power_zip)


# Diagnostics on the final model
sim_res_power <- simulateResiduals(model_power_zip, n = 500)
plot(sim_res_power)
testDispersion(sim_res_power)
testZeroInflation(sim_res_power)

# final model used downstream
model_power <- model_power_zip
# =========================
...
ggsave(
  "DownSamplingXPool_Figures/DietSignal_TPtoFP_Line_byPool.png",
  fig_tp_fp_lines,
  width = 14,
  height = 10,
  dpi = 300,
...
n_levels <- sort(unique(as.numeric(as.character(all_results_df$N_remaining))), decreasing = TRUE)
...
ggsave(
  "DownSamplingXPool_Figures/Fig_EffectSizeBin_DetectionRetention.png",
  fig_bin_detection,
  width = 12,
  height = 9,
  dpi = 300,
